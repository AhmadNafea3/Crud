...
if(existsResult.rows[0].exists){
    return res.status(409).json({error:`category ${req.body.name} alredy exixts`})
}
...
exports.ubdatecategory =async(req,res) =>{
   try{
    if(!req.body.name){
       return res.status(422).json({error:'Name is required'}) 
    }else{const existsResult =await database.pool.query({
   text:'SELECT EXISTS (SELECT * FROM category WHERE name=$1)',
    values:[req.body.name]
})
if(existsResult.rows[0].exists){
...
try{
    ...
    if(countresult.rows[0].count >0){
        return res.status(409).json({error:`category is being used in ${countresult.rows[0].count}`})
    }
const result= await database.pool.query({
